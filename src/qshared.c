...
int QDECL Com_sprintf(char *dest, int size, const char *fmt, ...)
{
	...
	if(len >= size)
	{
		...
		if(com_developer && com_developer->integer >= 3)
		{
			assert(0);
		}
	}
...
int QDECL Com_sprintfUni(wchar_t *dest, size_t size, const wchar_t *fmt, ...)
{
	...
	if(len < 0 || len >= numchar)
		Com_Printf(CON_CHANNEL_SYSTEM, "Com_sprintfUni: Output length %d too short\n", size);
...
void Info_RemoveKey( char *s, const char *key ) {
	...
	if ( strlen( s ) >= MAX_INFO_STRING ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: Info_RemoveKey: oversize infostring" );
	}
...
void BigInfo_RemoveKey( char *s, const char *key ) {
	...
	if ( strlen( s ) >= BIG_INFO_STRING ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: BigInfo_RemoveKey: oversize infostring" );
	}
...
char *Info_ValueForKey( const char *s, const char *key ) {
	...
	if ( strlen( s ) >= BIG_INFO_STRING ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: Info_ValueForKey: oversize infostring" );
		return "";
	}
...
char *Info_ValueForKey_tsInternal( const char *s, const char *key, char* buffer)
{
	...
	if ( strlen( s ) >= MAX_INFO_STRING ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: Info_ValueForKey_ts: oversize infostring" );
		return "";
	}
...
void Info_SetValueForKey( char *s, const char *key, const char *value ) {
	...
	if ( strlen( s ) >= MAX_INFO_STRING ) {
		Com_PrintWarning(CON_CHANNEL_ERROR, "Unexpected error - Info_SetValueForKey: oversize infostring" );
	}

	if (strchr (key, '\\'))
	{
		Com_PrintWarning (CON_CHANNEL_ERROR, "Can't use keys with a \\\n");
		Com_DPrintf(CON_CHANNEL_ERROR, "Bad key: %s value: %s\n", key, value);
		return;
	}

	if (strchr (value, '\\'))
	{
		Com_PrintWarning (CON_CHANNEL_ERROR, "Can't use values with a \\\n");
		Com_DPrintf(CON_CHANNEL_ERROR, "Bad value: %s key: %s\n", value, key);
		return;
	}

	if (strchr (key, ';'))
	{
		Com_PrintWarning (CON_CHANNEL_ERROR, "Can't use keys with a semicolon\n");
		Com_DPrintf(CON_CHANNEL_ERROR, "Bad key: %s value: %s\n", key, value);
		return;
	}
	...
	if (strchr (value, ';'))
	{
		Com_PrintWarning (CON_CHANNEL_ERROR, "Can't use values with a semicolon\n");
		Com_DPrintf(CON_CHANNEL_ERROR, "Bad value: %s key: %s\n", value, key);
		return;
	}
	...
	{
		Com_PrintWarning (CON_CHANNEL_ERROR, "Info string length exceeded\n");
		return;
	}
...
void BigInfo_SetValueForKey( char *s, const char *key, const char *value ) {
	...
	if ( strlen( s ) >= BIG_INFO_STRING ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: Info_SetValueForKey: oversize infostring" );
	}

	if (strchr (key, '\\') || strchr (value, '\\'))
	{
		Com_Printf(CON_CHANNEL_ERROR, "Error: Can't use keys or values with a \\\n");
		return;
	}

	if (strchr (key, ';') || strchr (value, ';'))
	{
		Com_Printf(CON_CHANNEL_ERROR, "Error: Can't use keys or values with a semicolon\n");
		return;
	}
	...
	{
		Com_Printf(CON_CHANNEL_ERROR, "Error: Info string length exceeded\n");
		return;
	}
...
void XML_AppendToBuffer( xml_t *base, const char* s )
{
    ...
    {
        Com_Printf(CON_CHANNEL_ERROR, "Error: XML_AppendToBuffer: Overflow!\n" );
        return;
    }
...
void XML_Init( xml_t *base, char *s, int size, char* encoding) {
	...
	if ( 256 > size ) {
		Com_Printf(CON_CHANNEL_ERROR, "Error: XML_Init: too small infostring" );
	}
...
void XML_CloseTag(xml_t *base) {
	...
	if(base->parents == 0)
	{
		Com_PrintError(CON_CHANNEL_ERROR, "XML_CloseTag: Invalid close attempt in XML. Attempt to close more elements than you have opened.\n");
		Com_Printf(CON_CHANNEL_ERROR, "Printing up to 960 recent characters of XML as debug:\n");
		...
		Com_PrintError(CON_CHANNEL_ERROR, "You have errors in your XML code\n");
		return;
	}
	base->parents--;
