...
#define mbedtls_printf _mbedtls_printf
...
void _mbedtls_printf(const char* fmt, ...)
{
	...
	Com_PrintMessage(CON_CHANNEL_FILEDL, msg, 0);
}
...
static qboolean HTTPSplitURL(const char* url, char* address, int lenaddress, char* wwwpath, int lenwwwpath)
{
	...
	{
		url += 7;
	}else if(!Q_stricmpn(url, "https://", 8)){
		url += 8;
		tls = qtrue;
	}
...
static qboolean HTTP_ParseRangeHeader(ftRequest_t* request, char* line)
{
	line += 14;
	...
	*l = 0;
	...
	request->rangeLow = atoi(low);
	request->rangeHigh = atoi(high);
	request->rangeTotLength = atoi(total);
	return qtrue;
}
  ...
  if(status < 0)
    ...
    request->version = 0;
		...
		while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf, sizeof(stringlinebuf))) && line[0] != '\0' && line[0] != '\r')
		{
			...
			}else if(!Q_stricmpn("Content-Range:", line, 14)){
				if(!HTTP_ParseRangeHeader(request, line))
				{
					request->contentLength = 0;
					return -1;
				}else{
					if(request->rangeLow >= request->rangeHigh)
					{
						return -1;
					}
				}
...
static int FTP_SendReceiveData(ftRequest_t* request)
{
  ...
  if(status == -2)
  {
	...
	switch(request->stage)
	{
      ...
      request->transferactive = qfalse;
      Com_DPrintf(CON_CHANNEL_SYSTEM, "FTP File transfer completed\n");
      return 1;
...
int HTTPServer_ReadMessage(netadr_t* from, msg_t* msg, ftRequest_t* request)
{
	...
	if(request->finallen == -1)
	{
		...
		if(Q_stricmpn(line,"HTTP/1.",7) || isInteger(line + 7, 2) == qfalse || isInteger(line + 9, 4) == qfalse)
		{
			Com_Printf(CON_CHANNEL_FILEDL, "HTTP_ReceiveData: Packet is corrupt!\nDebug: %s\n", line);
		...
		{
			Com_Printf(CON_CHANNEL_FILEDL, "HTTP_ReceiveData: Packet has unknown HTTP version 1.%d !\n", request->version);
		...
		while ((line = MSG_ReadStringLine(&request->recvmsg, stringlinebuf, sizeof(stringlinebuf))) && line[0] != '\0' && line[0] != '\r')
		{
			if(!Q_stricmpn("Content-Length:", line, 15))
			{
				if(isInteger(line + 15, 0) == qfalse)
				{
					Com_Printf(CON_CHANNEL_FILEDL, "Sec_GetHTTPPacket: Packet is corrupt!\nDebug: %s\n", line);
					return -1;
				}
				request->contentLength = atoi(line + 15);
				if(request->contentLength < 0)
