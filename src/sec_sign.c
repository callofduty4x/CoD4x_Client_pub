...
/*	if(Sec_Pem2Der(pemcert, strlen(pemcert), dercert, &dercertlen) != CRYPT_OK){
//		Com_PrintError(CON_CHANNEL_SYSTEM, "Pem to Der conversion failed\n");
		return qfalse;
	}
*/

	if(base64_decode((const unsigned char*)pemcert, strlen(pemcert), dercert, &dercertlen) != CRYPT_OK)
	{
		return qfalse;
	}
