...
{	
	...
	{
		Com_PrintError(CON_CHANNEL_SYSTEM, "Illegible reliable acknowledge - got: %d current: %d\n", acknowledge, chan->txwindow.acknowledge);
		return;
	}
	if(acknowledge < chan->txwindow.acknowledge)
	{
		Com_PrintError(CON_CHANNEL_SYSTEM, "Too old reliable acknowledge %d > %d\n", chan->txwindow.acknowledge, acknowledge);
		return;
	}
	if(acknowledge > chan->txwindow.sequence){
		Com_PrintError(CON_CHANNEL_SYSTEM, "Invalid reliable acknowledge. acknowledge(%d) > sequence(%d)\n", acknowledge, chan->txwindow.sequence);
		return;
	}
	...
	{
		Com_PrintError(CON_CHANNEL_SYSTEM, "Bad selective acknowledge count: %d\n", numselectiveack);	
		return;
	}
	for(i = 0; i < numselectiveack; ++i)
	{
		...
		if(startack + length > acknowledge + chan->txwindow.windowsize){
			Com_PrintError(CON_CHANNEL_SYSTEM, "Selective acknowledge %d is out of windowsize acknowledge %d\n", startack + length, acknowledge);
			return;
		}
#ifdef RELIABLE_DEBUG
		Com_Printf("SACK: %d %d\n", startack, length);
	...
	if(fragmentsize > MAX_FRAGMENT_SIZE){
		Com_PrintError(CON_CHANNEL_SYSTEM, "Invalid fragmentsize (%d)\n", fragmentsize);
		return;
	}
...
int ReliableMessageGetUsedFragmentCnt(netreliablemsg_t *chan)
{
	int usedfragmentcnt;
...
netreliablemsg_t* ReliableMessageSetup( int qport, int netsrc, netadr_t* remote)
{
	...
	if(chan == NULL)
	{
		Com_PrintError(CON_CHANNEL_ERROR, "ReliableMessageSetup(): Out of memory\n");
		return NULL;
	}
...
void ReliableMessageAddTestData(netreliablemsg_t *chan, int numint)
{
    ...
    sendpos += sentbytes;
    Com_Printf(CON_CHANNEL_SYSTEM, "Sending %d bytes...\n", sentbytes);
}
...
void ReliableMessageGetTestData(netreliablemsg_t *chan)
{
	...
	if(numbytes > 0)
		Com_Printf(CON_CHANNEL_SYSTEM, "Received %d bytes\n", numbytes);

	for(i = 0; i < (numbytes/4); ++i, ++verify){
		if(recvdata[i] != verify)
		{
			Com_Printf(CON_CHANNEL_SYSTEM, "Verify error! Expected: %d Got: %d\n", verify, recvdata[i]);	
		}
	}
