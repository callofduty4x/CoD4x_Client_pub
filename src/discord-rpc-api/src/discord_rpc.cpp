...
//    std::condition_variable waitForIOActivity;
    HANDLE waitForIOActivity;
    std::thread ioThread;

public:
    void Start()
    {
        waitForIOActivity = CreateEventA(NULL, false, true, NULL);
        if(waitForIOActivity == NULL)
        {
        ...
        ioThread = std::thread([&]() {
            ...
            while (keepRunning.load()) {
                ...
                WaitForSingleObject(waitForIOActivity, 500);
                Discord_UpdateConnection();
            }
        });
    }
...
extern "C" DISCORD_EXPORT void Discord_Shutdown(void)
{
    ...
    if (IoThread != nullptr) {
//        IoThread->Stop(); //Bad idea, because delete IoThread does already do this
        delete IoThread;
        IoThread = nullptr;
    }
